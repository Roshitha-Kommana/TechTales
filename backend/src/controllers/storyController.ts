...
import { processFile, isImageFile } from '../services/fileProcessor';
import { updateLearningStreakOnActivity } from './authController';
...
export const generateStoryController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let sourceFileContent: string | undefined;
    let sourceFileMimeType: string | undefined;
    let sourceFileBase64: string | undefined;
    ...
    if (file) {
      try {
        ...
        if (isImageFile(processedFile.mimeType)) {
    ...
    if (authReq.userId) {
      try {
        const streak = await updateLearningStreakOnActivity(authReq.userId);
...
export const getStoryController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const generateImagesController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const story = await Story.findById(id);
    ...
    const pagesWithImages = await Promise.all(
      story.pages.map(async (page, index) => {
        ...
        try {
          ...
          (page as any).imageError = errorMessage;
    ...
    const failedPagesDetails: Array<{ pageNumber: number; error?: string }> = [];
...
export const getAllStoriesController = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as AuthRequest;
...
export const saveStoryController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const story = await Story.findById(id);
...
export const updateStoryPageController = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as AuthRequest;
...
export const updateStoryPageImageController = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as AuthRequest;
    const { id, pageNumber } = req.params;
    const file = req.file;
...
export const getShareableLinkController = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const deleteStoryController = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as AuthRequest;
    const { id } = req.params;
