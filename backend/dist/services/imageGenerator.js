...
async function generateImagePrompt(storyText, concept, pageNumber, ageGroup = '8-12', storyTitle, adventureStyle = 'adventure') {
    ...
    Return ONLY the prompt description. No markdown.`;
        ...
        if (hasAction)
            sceneType = "action-packed scene";
        if (hasLearning)
            sceneType = "learning/discovery scene";
        if (hasTriumph)
            sceneType = "triumphant scene";
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            const modelName = useFallbackModel ? gemini_1.GEMINI_CONFIG.fallbackModel : gemini_1.GEMINI_CONFIG.storyModel;
            ...
            if (isQuota || isServer) {
                if (attempt === 0) {
                    ...
                    (0, gemini_1.rotateStoryKey)(); // Try rotating key
...
async function generateImage(imagePrompt, storyContext) {
    try {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                const response = await axios_1.default.get(POLLINATIONS_API_URL, {
