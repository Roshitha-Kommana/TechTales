...
async function generateStory(options) {
    const { concept, characterName = 'Adventurer', adventureStyle = 'adventure', difficulty = 'beginner', numberOfPages = 5, sourceFileContent, sourceFileMimeType, sourceFileBase64, } = options;
    // Log config on first call to debug
    console.log('🔧 GEMINI_CONFIG:', {
        storyModel: gemini_1.GEMINI_CONFIG.storyModel,
        fallbackModel: gemini_1.GEMINI_CONFIG.fallbackModel,
        hasStoryModel: !!gemini_1.GEMINI_CONFIG.storyModel,
        hasFallbackModel: !!gemini_1.GEMINI_CONFIG.fallbackModel,
    });
    // Map adventure styles to descriptions
    const adventureStyleDescriptions = {
    ...
    if (sourceFileContent || sourceFileBase64) {
        if (sourceFileBase64 && sourceFileMimeType && (0, fileProcessor_1.isImageFile)(sourceFileMimeType)) {
...
Return ONLY the JSON object, nothing else.`;
    ...
    let modelName = gemini_1.GEMINI_CONFIG.storyModel; // Declare outside loop for error handling
    // Validate model configuration
    if (!gemini_1.GEMINI_CONFIG.storyModel || !gemini_1.GEMINI_CONFIG.fallbackModel) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            modelName = useFallbackModel ? gemini_1.GEMINI_CONFIG.fallbackModel : gemini_1.GEMINI_CONFIG.storyModel;
            // Validate model name
            ...
            if (attempt === 4) {
                ...
                console.log(`🔄 Switching to fallback model: ${gemini_1.GEMINI_CONFIG.fallbackModel}`);
            }
            console.log(`📝 Generating story with model: ${modelName} (attempt ${attempt + 1}/${maxRetries})`);
            // Validate temperature exists
            const temperature = gemini_1.GEMINI_CONFIG.temperature?.story ?? 0.8;
            ...
            const model = gemini_1.geminiStory.getGenerativeModel(modelConfig);
            if (!model) {
            ...
            if (isQuotaError) {
                ...
                if (attempt < maxRetries - 1) {
                    ...
                    console.error('❌ All API keys exhausted after trying:', {
                        ...
                        modelsTried: [gemini_1.GEMINI_CONFIG.storyModel, gemini_1.GEMINI_CONFIG.fallbackModel],
