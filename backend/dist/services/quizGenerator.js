...
exports.calculateQuizAnalytics = calculateQuizAnalytics;
exports.generateQuizExplanations = generateQuizExplanations;
...
Return ONLY the JSON object, nothing else.`;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const modelName = useFallbackModel ? gemini_1.GEMINI_CONFIG.fallbackModel : gemini_1.GEMINI_CONFIG.quizModel;
            if (attempt === 2) {
            ...
            const model = gemini_1.geminiQuiz.getGenerativeModel({
                model: modelName,
                generationConfig: {
                    temperature: gemini_1.GEMINI_CONFIG.temperature.quiz,
                },
            });
...
function calculateQuizAnalytics(questions, results) {
    ...
    const areasOfConcern = incorrectQuestions.map(q => extractTopicFromQuestion(q));
    // Remove duplicates
    ...
    const correctQuestions = results
        ...
        .map(r => questions.find(q => q.questionNumber === r.questionNumber))
        .filter((q) => q !== undefined);
    const strengths = correctQuestions.map(q => extractTopicFromQuestion(q));
...
async function generateQuizExplanations(questions, concept) {
