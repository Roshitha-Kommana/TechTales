...
exports.getComprehensiveAnalyticsController = exports.getQuizAnalyticsController = exports.getQuizByStoryController = exports.getQuizController = exports.submitQuizController = exports.generateQuizController = void 0;
...
const generateQuizController = async (req, res) => {
    try {
        ...
        let quiz = await Quiz_1.Quiz.findOne({ storyId });
        ...
        const generatedQuiz = await (0, quizGenerator_1.generateQuiz)({
...
const submitQuizController = async (req, res) => {
    try {
        ...
        const questionsWithExplanations = await (0, quizGenerator_1.generateQuizExplanations)(quiz.questions, quiz.concept);
        quiz.questions = questionsWithExplanations;
        // Calculate analytics
        const analytics = (0, quizGenerator_1.calculateQuizAnalytics)(quiz.questions, results);
        ...
        if (authReq.userId) {
            ...
            await (0, leaderboardController_1.addUserPoints)(authReq.userId, totalPoints);
...
const getQuizByStoryController = async (req, res) => {
    try {
        const { storyId } = req.params;
        const quiz = await Quiz_1.Quiz.findOne({ storyId });
        if (!quiz) {
...
exports.getQuizByStoryController = getQuizByStoryController;
const getQuizAnalyticsController = async (req, res) => {
    try {
        const authReq = req;
        const query = authReq.userId
            ? { userId: authReq.userId, completedAt: { $exists: true } }
            : { completedAt: { $exists: true } };
        const quizzes = await Quiz_1.Quiz.find(query).sort({ completedAt: -1 });
        const analytics = {
...
exports.getQuizAnalyticsController = getQuizAnalyticsController;
const getComprehensiveAnalyticsController = async (req, res) => {
    try {
        const authReq = req;
        if (!authReq.userId) {
        ...
        const calculateSkillLevel = (avgScore, totalQuizzes) => {
            if (totalQuizzes === 0)
            ...
            if (skillPoints >= 90)
                return { level: 'Master', progress: Math.min((skillPoints - 90) / 10 * 100, 100) };
            if (skillPoints >= 70)
                return { level: 'Expert', progress: (skillPoints - 70) / 20 * 100 };
            if (skillPoints >= 50)
                return { level: 'Intermediate', progress: (skillPoints - 50) / 20 * 100 };
            if (skillPoints >= 30)
                return { level: 'Learner', progress: (skillPoints - 30) / 20 * 100 };
        ...
        if (totalStoriesRead >= 1)
            achievements.push({ id: 'first_story', name: 'First Story', description: 'Completed your first story', icon: '📖', earned: true });
        if (totalStoriesRead >= 5)
            achievements.push({ id: 'storyteller', name: 'Storyteller', description: 'Completed 5 stories', icon: '📚', earned: true });
        if (totalStoriesRead >= 10)
            achievements.push({ id: 'bookworm', name: 'Bookworm', description: 'Completed 10 stories', icon: '🐛', earned: true });
        if (averageQuizScore >= 80)
            achievements.push({ id: 'high_achiever', name: 'High Achiever', description: 'Average score above 80%', icon: '🏆', earned: true });
        if (averageQuizScore >= 90)
            achievements.push({ id: 'perfectionist', name: 'Perfectionist', description: 'Average score above 90%', icon: '⭐', earned: true });
        if ((user?.learningStreak || 0) >= 3)
            achievements.push({ id: 'streak_starter', name: 'Streak Starter', description: '3 day learning streak', icon: '🔥', earned: true });
        if ((user?.learningStreak || 0) >= 7)
            achievements.push({ id: 'week_warrior', name: 'Week Warrior', description: '7 day learning streak', icon: '💪', earned: true });
        if (weeklyQuizzes.length >= 5)
            achievements.push({ id: 'active_learner', name: 'Active Learner', description: '5 quizzes this week', icon: '📈', earned: true });
        // Add unearned achievements
        if (totalStoriesRead < 1)
            achievements.push({ id: 'first_story', name: 'First Story', description: 'Complete your first story', icon: '📖', earned: false });
        if (totalStoriesRead < 5)
            achievements.push({ id: 'storyteller', name: 'Storyteller', description: 'Complete 5 stories', icon: '📚', earned: false });
        if (averageQuizScore < 80)
            achievements.push({ id: 'high_achiever', name: 'High Achiever', description: 'Get average score above 80%', icon: '🏆', earned: false });
        if ((user?.learningStreak || 0) < 3)
            achievements.push({ id: 'streak_starter', name: 'Streak Starter', description: 'Maintain 3 day streak', icon: '🔥', earned: false });
        // === NEW: Areas of Concern Summary ===
        const allAreasOfConcern = {};
        quizzes.forEach(q => {
            if (q.areasOfConcern) {
                q.areasOfConcern.forEach(area => {
...
exports.getComprehensiveAnalyticsController = getComprehensiveAnalyticsController;
