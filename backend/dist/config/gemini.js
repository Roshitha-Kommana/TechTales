"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GEMINI_CONFIG = exports.geminiImage = exports.rotateImageKey = exports.getImageClient = exports.geminiQuiz = exports.geminiStory = exports.rotateQuizKey = exports.getQuizClient = exports.rotateStoryKey = exports.getStoryClient = void 0;
const generative_ai_1 = require("@google/generative-ai");
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
// API Key 1: For story generation and image prompt generation
const GEMINI_API_KEY_1 = process.env.GEMINI_API_KEY_1 || 'AIzaSyBL8Z_qif_1Scu71G1v3urGry9PgxoXR3c';
// API Key 2: For quiz generation (can be used as fallback)
const GEMINI_API_KEY_2 = process.env.GEMINI_API_KEY_2 || 'AIzaSyB30HfF-nBO3go6SWXHx-uT88J2p-2ugnk';
// API Key 3: For image generation
const GEMINI_API_KEY_3 = process.env.GEMINI_API_KEY_3 || 'AIzaSyANxQZIGVpFBKRMzkDTEYAQ96dZ6xgJFfY';
// API Key 4: Additional key for increased quota capacity
const GEMINI_API_KEY_4 = process.env.GEMINI_API_KEY_4 || '';
if (!GEMINI_API_KEY_1 || !GEMINI_API_KEY_2) {
    throw new Error('GEMINI_API_KEY_1 and GEMINI_API_KEY_2 must be set in environment variables');
}
// Create clients for all keys
const client1 = new generative_ai_1.GoogleGenerativeAI(GEMINI_API_KEY_1);
const client2 = new generative_ai_1.GoogleGenerativeAI(GEMINI_API_KEY_2);
const client3 = new generative_ai_1.GoogleGenerativeAI(GEMINI_API_KEY_3);
const client4 = GEMINI_API_KEY_4 ? new generative_ai_1.GoogleGenerativeAI(GEMINI_API_KEY_4) : null;
// Track which key to use (for rotation)
let currentStoryKey = 2; // Start with Key 2 since Key 1 quota is exhausted
let currentQuizKey = 2; // Start with Key 2 for quizzes
let currentImageKey = 2; // Start with Key 2 for image prompt generation
// Helper function to get story client with automatic key rotation on quota errors
const getStoryClient = () => {
    if (currentStoryKey === 1)
        return client1;
    if (currentStoryKey === 2)
        return client2;
    if (currentStoryKey === 3)
        return client3;
    if (currentStoryKey === 4 && client4)
        return client4;
    return client3; // Fallback to key 3 if key 4 not available
};
exports.getStoryClient = getStoryClient;
// Helper function to rotate story API key (cycles through all available keys: 1 -> 2 -> 3 -> 4 -> 1)
const rotateStoryKey = () => {
    if (currentStoryKey === 1) {
        currentStoryKey = 2;
    }
    else if (currentStoryKey === 2) {
        currentStoryKey = 3;
    }
    else if (currentStoryKey === 3) {
        currentStoryKey = client4 ? 4 : 1; // Use key 4 if available, otherwise cycle back
    }
    else {
        currentStoryKey = 1; // Cycle back to first key
    }
    console.log(`🔄 Rotated to API Key ${currentStoryKey} for story generation`);
};
exports.rotateStoryKey = rotateStoryKey;
// Helper function to get quiz client with automatic key rotation
const getQuizClient = () => {
    if (currentQuizKey === 1)
        return client1;
    if (currentQuizKey === 2)
        return client2;
    if (currentQuizKey === 3)
        return client3;
    if (currentQuizKey === 4 && client4)
        return client4;
    return client3; // Fallback to key 3 if key 4 not available
};
exports.getQuizClient = getQuizClient;
// Helper function to rotate quiz API key (cycles through all available keys: 1 -> 2 -> 3 -> 4 -> 1)
const rotateQuizKey = () => {
    if (currentQuizKey === 1) {
        currentQuizKey = 2;
    }
    else if (currentQuizKey === 2) {
        currentQuizKey = 3;
    }
    else if (currentQuizKey === 3) {
        currentQuizKey = client4 ? 4 : 1; // Use key 4 if available, otherwise cycle back
    }
    else {
        currentQuizKey = 1; // Cycle back to first key
    }
    console.log(`🔄 Rotated to API Key ${currentQuizKey} for quiz generation`);
};
exports.rotateQuizKey = rotateQuizKey;
// Gemini client for story and image prompts (with rotation support)
exports.geminiStory = {
    getGenerativeModel: (config) => {
        return (0, exports.getStoryClient)().getGenerativeModel(config);
    }
};
// Gemini client for quiz generation (with rotation support)
exports.geminiQuiz = {
    getGenerativeModel: (config) => {
        return (0, exports.getQuizClient)().getGenerativeModel(config);
    }
};
// Helper function to get image client with automatic key rotation
const getImageClient = () => {
    if (currentImageKey === 1)
        return client1;
    if (currentImageKey === 2)
        return client2;
    return client3;
};
exports.getImageClient = getImageClient;
// Helper function to rotate image API key
const rotateImageKey = () => {
    if (currentImageKey === 3) {
        currentImageKey = 1;
    }
    else if (currentImageKey === 1) {
        currentImageKey = 2;
    }
    else {
        currentImageKey = 3;
    }
    console.log(`🔄 Rotated to API Key ${currentImageKey} for image generation`);
};
exports.rotateImageKey = rotateImageKey;
// Gemini client for image generation (with rotation support)
exports.geminiImage = {
    getGenerativeModel: (config) => {
        return (0, exports.getImageClient)().getGenerativeModel(config);
    }
};
exports.GEMINI_CONFIG = {
    storyModel: 'gemini-2.5-flash',
    quizModel: 'gemini-2.5-flash',
    imageModel: 'gemini-2.5-flash',
    fallbackModel: 'gemini-2.5-flash-lite',
    temperature: {
        story: 0.8,
        quiz: 0.7,
        prompt: 0.7,
        image: 0.8,
    },
};
//# sourceMappingURL=gemini.js.map